...
Created Summer 2024 for LIBR 599C, Intro to Python
...
#### Features:
The game engine and object scripting support the following features:
...
- **simple conversations with npcs** that allow the player to talk to creatures and humanoids and ask or say different things. Responses are defined externally, but through regex, answers can be somewhat on topic.
...
If either the player’s or the monster’s HP is 0 or less, they die.  
...
- **add** _attribute_ _value_ - increment the attribute by the integer amount in value. The script parser checks to see if the attribute belongs to the _item_, if not, checks to see if _player_ has that attribute, and failing that, checks to see if the current _room_ has that attribute.
...
- **sub** _attribute_ _value_ - decrement the attribute by the integer amount in value. The script parser checks to see if the attribute belongs to the _item_, if not, checks to see if _player_ has that attribute, and failing that, checks to see if the current _room_ has that attribute.
